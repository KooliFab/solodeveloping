...
![Économies de coûts Firebase avec le lazy loading Flutter](/images/blog/lazy-loading/flutter-lazy-loading-money.gif)
...
## Implémentation du Lazy Loading sur Flutter
...
![Application Flutter implémentant la pagination lazy loading avec Firebase](/images/blog/lazy-loading/flutter-app-with-lazy-loading.gif)
...
![Lectures élevées de la base de données Firebase avant optimisation](/images/blog/lazy-loading/firebase-reads-before-lazy-loading.webp)
...
![Lectures réduites de la base de données Firebase après optimisation](/images/blog/lazy-loading/firebase-reads-after-lazy-loading.webp)
...
En résumé, le lazy loading s'est avéré très efficace pour notre application. Non seulement il nous a permis de rester dans le plan gratuit de Firebase, mais il nous a aussi donné de meilleures marges d'évolution.
...
![Implémentation réussie du lazy loading avec Flutter et Firebase](/images/blog/lazy-loading/flutter-lazy-loading-success.gif)
`,
    },
  },
  {
...
![Flutter multi-platform development for iOS, Android, and Web](/images/blog/pwa-flutter/flutter-pwa-multi-platform.webp)
...
![How a Progressive Web App (PWA) works compared to native apps](/images/blog/pwa-flutter/flutter-pwa-diagram.webp)
...
![Developer enthusiastic about using Flutter for the web](/images/blog/pwa-flutter/flutter-pwa-yes-mccallister.gif)
...
![Flutter PWA demonstration running smoothly from a web browser](/images/blog/pwa-flutter/flutter-pwa-web-browser-demo.gif)
...
![Flutter Web PWA performance and constraints](/images/blog/pwa-flutter/flutter-pwa-performance.webp)
...
![Deploying a Flutter PWA to Firebase Hosting](/images/blog/pwa-flutter/flutter-pwa-deployment.webp)
...
- **Financial and operational savings:** The absence of license fees ($99/year for Apple, $25 for Google) may seem anecdotal, but it's especially the saving of resources related to preparing store submissions that proved significant.
...
- **Absence from official stores:** In a word-of-mouth context, we found that new users naturally looked for the application on the App Store or Play Store before realizing it was accessible only via the web. We estimate that this absence from official distribution platforms represents a loss of organic acquisition of about 15% compared to a traditional application.

![Confused user searching for the Flutter PWA on the Apple App Store](/images/blog/pwa-flutter/flutter-pwa-where-is-it-travolta.gif)

- **Installation friction:** Our statistics show that only 23% of regular users have actually "installed" the application, others using it directly via the browser.
- **Technical limitations of native APIs:** Permission management proved to be problematic in certain contexts. Browsers can sometimes malfunction depending on user actions, preventing the use of certain essential functionalities like camera or geolocation.

![Web browser API permissions in a Flutter PWA](/images/blog/pwa-flutter/flutter-pwa-permissions.webp)
...
![SpongeBob saying see you soon](/images/blog/pwa-flutter/flutter-pwa-spongebob.gif)
      `,
      fr: `
...
![Performances et contraintes de Flutter Web PWA](/images/blog/pwa-flutter/flutter-pwa-performance.webp)
...
![Architecture web de Flutter compilant Dart en JavaScript pour les PWA](/images/blog/pwa-flutter/flutter-pwa-architecture.webp)
...
![Flux de travail agile grâce au déploiement rapide des PWA Flutter](/images/blog/pwa-flutter/flutter-pwa-workflow.webp)
...
![Permissions des API web sur navigateur dans une PWA Flutter](/images/blog/pwa-flutter/flutter-pwa-permissions.webp)
...
![Bob l'éponge disant à bientôt](/images/blog/pwa-flutter/flutter-pwa-spongebob.gif)
  ...
  },
  {
    ...
    title: {
      ...
      fr: "Why I Don't Build SaaS Apps Around AI API Keys",
    },
    subtitle: {
      en: "Business risk, big tech competition, security, and cost — the reasons behind my choice",
      fr: "Business risk, big tech competition, security, and cost — the reasons behind my choice",
    },
    date: "2026-02-24",
    author: "Fabien Chung",
    coverImage: "/images/blog/pwa-flutter/flutter-pwa-hero.webp",
    content: {
      en: `
Everyone is building AI SaaS right now. Wrap an API call to OpenAI, add a nice UI on top, charge a subscription, and ship it. The playbook seems simple, and new products pop up every week — AI copywriters, AI schedulers, AI summarizers, AI everything.

I'm not doing it. Here's why.

## You're Building on Someone Else's Foundation

When your entire product is a wrapper around a third-party AI model, you don't own anything. You don't own the model. You don't own the inference. You don't even own the pricing.

OpenAI, Anthropic, Google, Meta — these companies move fast. What takes an indie developer months to build, package, and market, a big tech company can ship as a native feature in a single update. We've seen it happen repeatedly: standalone AI writing tools getting absorbed by Notion, Google Docs, and every major text editor. Dedicated AI image generators becoming a button inside Canva and Adobe.

The moment your value proposition is "we call the same API as everyone else, but our UI is nicer," you're one product update away from irrelevance. There is no moat. There is no defensible advantage. You are renting your core feature from a company that has no obligation to keep things the way they are.

API versions get deprecated. Pricing changes overnight. Rate limits shift. Terms of service evolve. Your entire business model sits on ground you don't control.

## The Business Risk Is Real Before You Even Have Users

Here's the part that bothers me the most: cost comes before revenue.

With a traditional SaaS, your infrastructure costs are relatively predictable and often marginal during the early stages. A database, a server, maybe a CDN. You can run lean while you figure out if people actually want what you're building.

With an AI-powered SaaS built on API keys, every single user interaction costs money. Every prompt, every completion, every token. You're burning cash from day one, before you've validated that anyone is willing to pay for your product.

And the economics don't scale in your favor. If your product gets traction, your API bill explodes. If it doesn't, you've spent money on inference for users who were never going to convert. Either way, you're spending before you know if the market even exists.

I prefer to spend as little as possible before I have proof that something works. That's not being cheap — it's being smart with limited resources.

## A Security Surface You Don't Control

Integrating a third-party AI API means introducing a security surface that you cannot fully govern.

API keys in production are a liability. They can be leaked through client-side code, logged accidentally, or extracted from reverse-engineered mobile apps. Once compromised, someone else runs up your bill.

Then there's prompt injection — a category of vulnerability that is still not fully solved. If your product lets users interact with an LLM, you're exposed to manipulation techniques that can bypass your instructions, extract system prompts, or produce outputs you never intended.

And all of this runs through infrastructure you don't operate. You trust the provider's uptime, their data handling, their compliance posture. For a solo developer or a small team, that's a lot of trust placed in a single dependency.

## What I Do Instead

I'm not against AI. I use AI tools every day as a developer. But I draw a clear line between using AI as a tool and building a business whose entire value depends on an API key.

**Validate the problem first, without AI.** If the core problem your product solves requires AI to exist, ask yourself: is the problem real, or is the technology just exciting? Most of the time, you can validate demand with a simpler version that doesn't cost you per request.

**Add AI as a feature, not the product.** If AI genuinely improves the user experience, integrate it as one feature among many — not as the foundation. This way, if the API changes, your product still stands.

**Consider open-source and local models.** When AI is truly essential, look at open-source models you can self-host. You trade convenience for control, but you own your inference pipeline, your costs become predictable, and you're not one pricing change away from a margin crisis.

**Focus on what you can own.** Your data model, your user experience, your domain expertise, your distribution — these are things no API provider can take from you. Build your moat there.

---

The AI gold rush is real, and I understand the temptation. But I've learned that the best businesses are built on foundations you control. Wrapping an API key is fast, but fast doesn't mean durable.

I'd rather build something slower that I actually own.
      `,
    },
  },
];
